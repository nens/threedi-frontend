...
angular.module("threedi-client")
  .controller("EditSettings", ["$scope", "clientState", "state", "socket",
  function($scope, clientstate, state, socket){
    ...
    $scope.update_from_server = function() {
      ...
      if (($scope.radar_dt_server !== undefined) &&
        ...
        $scope.radar_dt = moment_date.toDate();
      }
    ...
    $scope.set_rain_type = function(new_type) {
      // Stop simulation
      if (state.master === true) {
    ...
    $scope.$watch('clientstate.edit_ranges.flood_fill_mode.value', function() {
      ...
      if ((clientstate.program_mode === MODE_FLOODFILL_ABSOLUTE) ||
        (clientstate.program_mode === MODE_FLOODFILL_RELATIVE)) {
          if (clientstate.edit_ranges['flood_fill_mode'].value === 0){
            clientstate.setMode(MODE_FLOODFILL_RELATIVE);
          } else if (clientstate.edit_ranges['flood_fill_mode'].value === 1) {
            clientstate.setMode(MODE_FLOODFILL_ABSOLUTE);
          }
        }
    });
